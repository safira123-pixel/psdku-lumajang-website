...
const item_pengumuman8 = (props) => {
  ...
  useEffect(() => {
    fetch(config.apiUrl + "/api/berita") // Pastikan URL endpoint sesuai
              ...
              <div className={classes.container}>
                {data.map((item, index) => (
                  <Breadcrumb key={index} style={{ marginTop: "10px" }}>
                    ...
                    <Breadcrumb.Item href="/pengumuman1">
                      {" "}
                      {t("Kegiatan Mahasiswa")}
                    </Breadcrumb.Item>
                    <Breadcrumb.Item active href="/item_pengumuman2">
                      {" "}
                      {item[1].name}
                    </Breadcrumb.Item>
                  </Breadcrumb>
            ...
            <Grid container className={classes.contentContainer}>
              <Grid item className={classes.gridItemFix} xs={12} sm={4} lg={9}>
                {data.map((item, index) => (
                  <NewsCard
                    key={index}
                    profileName={item[7].name}
                    profileImg={item[7].data}
                    content1={item[7].description}
                  />
                ))}
              </Grid>
...
export default withRoot(
  withStyles(styles)(withTranslation()(item_pengumuman8))
);
