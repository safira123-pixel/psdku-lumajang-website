...
import NewsCard from "../../components/NewsCard_kegiatanMahasiswa2";
...
const kegiatan_mahasiswa2 = (props) => {
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState([]);
  const { classes } = props;
  const { t } = useTranslation();
  useEffect(() => {
    fetch(config.apiUrl + "/api/kegiatan") // Pastikan URL endpoint sesuai
            ...
            <Card className={classes.card}>
              <div className={classes.container}>
                <Breadcrumb style={{ marginTop: "10px" }}>
                  ...
                  <Breadcrumb.Item href="/kegiatan_mahasiswa">
                    {" "}
                    {t("Kegiatan Mahasiswa")}
                  </Breadcrumb.Item>
                  <Breadcrumb.Item active href="/kegiatan_mahasiswa2">
                    {" "}
                    {t("Kegiatan Mahasiswa")}
                  </Breadcrumb.Item>
                </Breadcrumb>
              ...
              <Grid item className={classes.gridItemFix} xs={12} sm={4} lg={9}>
                {data.map((item, index) => (
                  <NewsCard
                    ...
                    data1={item[3].data}
                    nama1={item[3].name}
                    deskripsi1={item[3].selengkapnya}
                    link2={"/item_kegiatan5"}
                    content2={t("Selengkapnya")}
                    data2={item[4].data}
                    nama2={item[4].name}
                    deskripsi2={item[4].selengkapnya}
                    link3={"/item_kegiatan6"}
                    content3={t("Selengkapnya")}
                    data3={item[5].data}
                    nama3={item[5].name}
                    deskripsi3={item[5].selengkapnya}
                  />
                ))}
              </Grid>
...
export default withRoot(
  withStyles(styles)(withTranslation()(kegiatan_mahasiswa2))
);
